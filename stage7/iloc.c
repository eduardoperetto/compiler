...
void print_arg(ilocArg *arg) {
  if (arg == NULL) return;
  char* arg_str = arg_to_string(arg);
  printf("%s", arg_str);
  free(arg_str);
}

char* arg_to_string(ilocArg *arg) {
    if (arg == NULL) return NULL;

    char *result = (char*)malloc(256); // Alocar um buffer inicial
    if (result == NULL) {
        return NULL; // Falha na alocação
    }

    if (arg->temp_reg != NULL) {
        snprintf(result, 256, "%s", arg->temp_reg);
    } else if (arg->label != NULL) {
        snprintf(result, 256, "%s", arg->label);
    } else {
        snprintf(result, 256, "%d", arg->imediate_value);
    }
...
void print_code(ilocCode *code) {
  char* code_str = get_code_as_string(code);
  printf("%s", code_str);
  free(code_str);
}

char* get_code_as_string(ilocCode *code) {
    ilocCode *current = code;
    size_t buffer_size = 1024; // Tamanho inicial do buffer
    char *result = (char*)malloc(buffer_size);
    if (result == NULL) {
        return NULL; // Falha na alocação
    }
    
    result[0] = '\0'; // Iniciar a string vazia

    while (current != NULL) {
        char temp[256]; // Buffer temporário para cada linha
        switch (current->operation) {
            case NOP:
                snprintf(temp, sizeof(temp), "%s: nop\n", (current->arg1)->label);
                break;

            case JUMP: // OP -> arg1
            case JUMPI:
                snprintf(temp, sizeof(temp), "%s -> ", get_operation_string(current->operation));
                strcat(temp, arg_to_string(current->arg1)); // Presumindo que print_arg_to_string retorne char*
                strcat(temp, "\n");
                break;
            ...
            case CSTOREAO:
                snprintf(temp, sizeof(temp), "%s ", get_operation_string(current->operation));
                strcat(temp, arg_to_string(current->arg1));
                strcat(temp, " => ");
                strcat(temp, arg_to_string(current->arg2));
                if (current->arg3 != NULL) {
                    strcat(temp, ", ");
                    strcat(temp, arg_to_string(current->arg3));
                }
                strcat(temp, "\n");
                break;
            ...
            case HALT:
                snprintf(temp, sizeof(temp), "halt\n");
                strcat(result, temp);
                return result;
